/**
 * Fitness Agent SubGraph - ReAct模式
 */
...
/**
 * Fitness Agent的内部状态 - ReAct模式
 */
const FitnessState = Annotation.Root({
  input: Annotation({
    reducer: (x, y) => y ?? x,
    default: () => ({})
  }),
...
// Fitness Agent的LLM
const fitnessLLM = new ChatOpenAI({
  configuration: {
...
async function react_thought_node(state) {
  ...
  logger.info('Fitness ReAct Thought节点', { iteration: iterations });
  ...
  const systemPrompt = `你是健身专家助手，使用ReAct模式推理。
...
现在请思考:`;
  ...
  try {
    const response = await fitnessLLM.invoke([
    ...
    if (content.includes('Final Answer:')) {
      ...
      logger.info('Fitness ReAct完成', { iterations: iterations + 1 });
    ...
    if (!actionInputMatch) {
      logger.warn('Fitness ReAct无法解析Action Input，直接完成');
      // 如果有知识，就用现有知识回答
      if (knowledgeBase.length > 0) {
        return {
          thoughts: [thought],
    ...
    logger.info('Fitness ReAct解析Action', { query: actionInput.query });
    ...
    logger.error('Fitness ReAct Thought失败', { error: error.message });
    return {
      isDone: true,
      response: '抱歉，分析过程出错。',
      iterations: iterations + 1
...
async function react_retrieve_node(state) {
  ...
  logger.info('Fitness ReAct Retrieve节点', { query: ragQuery });

  try {
    const results = await hybridRetriever.retrieve(ragQuery, 'fitness');

    logger.info('Fitness ReAct检索完成', { count: results.length });
    ...
    logger.error('Fitness ReAct检索失败', { error: error.message });
    return { knowledgeBase: [] };
  }
}
...
async function final_reasoning_node(state) {
  ...
  try {
    ...
    // 替换knowledge占位符
    const systemPrompt = fitnessPrompt.replace('{knowledge}', knowledge);
    ...
    const response = await fitnessLLM.invoke(messages);

    logger.info('Fitness最终推理完成');

    return { response: response.content };

  } catch (error) {
    logger.error('Fitness最终推理失败', { error: error.message });
    return { response: '抱歉，生成运动建议时出现错误。' };
  }
}
...
function shouldContinue(state) {
  ...
  if (isDone) {
    ...
    if (state.knowledgeBase && state.knowledgeBase.length > 0 && !state.response) {
      logger.info('Fitness ReAct转到最终推理');
      return 'final_reasoning';
    }
    logger.info('Fitness ReAct结束', { reason: 'isDone', iterations });
    return END;
  }

  if (iterations >= 3) {
    logger.info('Fitness ReAct达到最大迭代', { iterations });
    return 'final_reasoning';
  }
...
/**
 * 创建Fitness ReAct SubGraph
 */
function createFitnessSubGraph() {
...
module.exports = { createFitnessSubGraph };
