...
void TcpKernel::ClearUserByFd(sock_fd clientfd)
{
    for (map<int, int>::iterator ite = m_mapIDToUserFD.begin();
         ite != m_mapIDToUserFD.end(); ++ite)
    {
        if (ite->second == clientfd)
        {
            m_mapIDToUserFD.erase(ite);
            return;
        }
    }
}
